  ...
  function getCert() {
    ...
    for (const key of keys) {
      try {
        ...
        if (d.certificate && (d.certificate.certId || d.certificate.score)) return d.certificate;
      } catch(e) {}
    }
    ...
    try {
      ...
      if (p.lastScore) {
        const cert = {
          candidateName: u.name || 'LedgerLearn Student',
          certTitle:     'Xero Certified Practitioner — Level 1',
          certLevel:     'L1 · Associate · Xero Cloud Accounting',
        ...
        // Save it back so future calls find it
        p.certificate = cert;
  ...
  function wireButtons() {
    document.querySelectorAll('button, a[onclick]').forEach(el => {
      ...
      if (isDownload) {
        ...
        el.addEventListener('click', function(e) { e.preventDefault(); e.stopPropagation(); downloadCertificate(); }, true);
      }
      if (isLinkedIn) {
        el.removeAttribute('onclick');
        el.addEventListener('click', function(e) { e.preventDefault(); e.stopPropagation(); shareLinkedIn(); }, true);
      }
    });
  }

  // Run on load and watch for DOM changes (results card appears after assessment)
